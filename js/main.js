...
function toggleAccordion(id) {
  ...
  if (!isOpen) {
    ...
    // Lancer l'audio automatiquement après l'animation (si mode auto)
    setTimeout(() => {
...
function toggleAccSettings(accId) {
  ...
  document.querySelectorAll('.acc-settings-btn.active').forEach(b => b.classList.remove('active'));
  ...
  if (!isOpen) {
    ...
    // Positionner le panel près du bouton
    const acc = document.getElementById(accId);
    const accRect = acc ? acc.getBoundingClientRect() : null;
    if (accRect) {
      panel.style.top = (accRect.top + window.scrollY + 50) + 'px';
  ...
  if (!isOpen) {
    if (btn) {
      ...
      const doShow = () => {
        ...
        const spaceAbove = r2.top - topBarH - 8;
        if (spaceAbove >= panelH) {
          // Assez de place au-dessus du bouton
          panel.style.top = (r2.top + window.scrollY - panelH - 8) + 'px';
        } else {
          // Fallback : sous la topbar
      ...
      // Afficher le panel directement au-dessus du bouton
      doShow();
    }
  }
}
...
function injectAccordionControls() {
  ...
  document.querySelectorAll('.accordion').forEach(acc => {
    ...
    actions.innerHTML =
      `<a class="acc-back-btn" href="${backHref}">` +
        `<div class="acc-back-circle">&#8592;</div>` +
        `<span class="acc-back-lbl">Retour</span>` +
      `</a>` +
      `<div class="acc-right-btns">` +
