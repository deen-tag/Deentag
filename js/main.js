...
function toggleReorder() {
  ...
  // Changer l'icône : crayon → check
  const svgPaths = btn.querySelector('.reorder-circle svg');
  if (isReorderMode) {
    svgPaths.innerHTML = '<polyline points="20 6 9 17 4 12" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>';
  } else {
    svgPaths.innerHTML = '<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>';
    saveOrder();
  }
...
function initDragCard(card) {
  ...
  card.ondragover = (e) => {
    e.preventDefault();
    if (card !== dragSrc && !card.classList.contains('coming-soon')) {
  ...
  card.ondrop = (e) => {
    ...
    if (dragSrc && dragSrc !== card && !card.classList.contains('coming-soon')) {
      swapCards(dragSrc, card);
    }
  };
  card.ondragend = () => {
    card.classList.remove('dragging', 'drag-over');
    document.querySelectorAll('.icon-card').forEach(c => c.classList.remove('drag-over'));
    dragSrc = null;
  };
  ...
  card.ontouchmove = (e) => {
    ...
    const target = el ? el.closest('.icon-card') : null;
    document.querySelectorAll('.icon-card').forEach(c => c.classList.remove('drag-over'));
...
function swapCards(a, b) {
  ...
  if (ia < ib) {
    grid.insertBefore(a, b.nextSibling);
    grid.insertBefore(b, cards[ia]);
  } else {
    grid.insertBefore(b, a.nextSibling);
    grid.insertBefore(a, cards[ib]);
  }
}
