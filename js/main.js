...
function applyLang(lang) {
  ...
  // Update pill labels
  document.querySelectorAll('.size-pill[data-size-val="small"]').forEach(el => {
    el.textContent = i18n[lang]['size-small'] || 'Petit';
  });
  document.querySelectorAll('.size-pill[data-size-val="medium"]').forEach(el => {
    el.textContent = i18n[lang]['size-medium'] || 'Moyen';
  });
  document.querySelectorAll('.size-pill[data-size-val="large"]').forEach(el => {
    el.textContent = i18n[lang]['size-large'] || 'Grand';
  });
}
...
const SIZE_LEVELS = {
  fr:  { small: 14, medium: 17, large: 22 },
  ph:  { small: 12, medium: 14, large: 18 },
  ar:  { small: 20, medium: 26, large: 34 },
  sn:  { small: 11, medium: 13, large: 16 },
};
...
function injectAccordionControls() {
  ...
  document.querySelectorAll('.accordion').forEach(acc => {
      ...
      bar.innerHTML =
        '<div class="audio-player-bar">' +
          ...
          '<div class="audio-center-label">' +
            ...
            '<span class="audio-center-title">Écouter</span>' +
            '<span class="audio-center-sub">les pistes</span>' +
          '</div>' +
    ...
    let pillsHTML = '<div class="settings-panel-title">Taille du texte</div>';

    if (!hasSunnah) {
      const types = [
        { key: 'fr', labelKey: 'size-fr' },
        { key: 'ph', labelKey: 'size-ph' },
        { key: 'ar', labelKey: 'size-ar' },
      ];
      types.forEach(({ key, labelKey }) => {
        const curLevel = getCurrentSizeLevel(key);
        pillsHTML +=
          '<div class="size-row">' +
            '<span class="size-row-label">' + (i18n[lang][labelKey] || key.toUpperCase()) + '</span>' +
            '<div class="size-pills">' +
              '<button class="size-pill' + (curLevel === 'small' ? ' active' : '') + '" data-size-type="' + key + '" data-size-val="small" onclick="event.stopPropagation();applySizeLevel(\'' + key + '\',\'small\')">' + (i18n[lang]['size-small'] || 'Petit') + '</button>' +
              '<button class="size-pill' + (curLevel === 'medium' ? ' active' : '') + '" data-size-type="' + key + '" data-size-val="medium" onclick="event.stopPropagation();applySizeLevel(\'' + key + '\',\'medium\')">' + (i18n[lang]['size-medium'] || 'Moyen') + '</button>' +
              '<button class="size-pill' + (curLevel === 'large' ? ' active' : '') + '" data-size-type="' + key + '" data-size-val="large" onclick="event.stopPropagation();applySizeLevel(\'' + key + '\',\'large\')">' + (i18n[lang]['size-large'] || 'Grand') + '</button>' +
            '</div>' +
          '</div>';
      });
    } else {
      const curLevel = getCurrentSizeLevel('sn');
      pillsHTML +=
        '<div class="size-row">' +
          '<span class="size-row-label">Texte</span>' +
          '<div class="size-pills">' +
            '<button class="size-pill' + (curLevel === 'small' ? ' active' : '') + '" data-size-type="sn" data-size-val="small" onclick="event.stopPropagation();applySizeLevel(\'sn\',\'small\')">' + (i18n[lang]['size-small'] || 'Petit') + '</button>' +
            '<button class="size-pill' + (curLevel === 'medium' ? ' active' : '') + '" data-size-type="sn" data-size-val="medium" onclick="event.stopPropagation();applySizeLevel(\'sn\',\'medium\')">' + (i18n[lang]['size-medium'] || 'Moyen') + '</button>' +
            '<button class="size-pill' + (curLevel === 'large' ? ' active' : '') + '" data-size-type="sn" data-size-val="large" onclick="event.stopPropagation();applySizeLevel(\'sn\',\'large\')">' + (i18n[lang]['size-large'] || 'Grand') + '</button>' +
          '</div>' +
        '</div>';
    }
