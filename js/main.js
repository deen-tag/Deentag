...
function openInvOverlay(accId) {
  ...
  const isSunnah = !!document.querySelector('#' + accId + ' .sunnah-item');
  ...
  if (!isSunnah) {
    ...
    const accEl = document.getElementById(accId);
    ['fr','en','es'].forEach(l => {
      const block = accEl ? accEl.querySelector('.lang-block[data-lang-block="' + l + '"]') : null;
      if (!block) return;
      html += '<div class="lang-block" data-lang-block="' + l + '">';
      block.querySelectorAll('.sunnah-item').forEach(item => {
