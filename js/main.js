...
// ===== SÉLECTEUR COULEUR THÈME (temporaire — test visuel) =====
const COLOR_THEMES = ['gold', 'olive', 'taupe', 'dune', 'ardoise'];
...
function applyColorTheme(name) {
  ...
  document.querySelectorAll('.color-theme-dot').forEach(dot => {
    dot.classList.toggle('active', dot.dataset.colorTheme === name);
  });
}

function injectColorThemePicker() {
  // Évite double injection
  if (document.querySelector('.color-theme-picker')) return;

  const picker = document.createElement('div');
  picker.className = 'color-theme-picker';
  picker.style.cssText = 'display:flex;align-items:center;gap:6px;';

  COLOR_THEMES.forEach(theme => {
    const dot = document.createElement('button');
    dot.className = 'color-theme-dot';
    dot.setAttribute('data-color-theme', theme);
    dot.title = theme.charAt(0).toUpperCase() + theme.slice(1);
    dot.addEventListener('click', () => applyColorTheme(theme));
    picker.appendChild(dot);
  });

  // Injection dans la topbar — cherche index-topbar ou top-bar ou inv-topbar
  const topbar = document.querySelector('.index-topbar, .top-bar, .inv-topbar');
  if (topbar) {
    // Insère après le premier enfant (bouton kids ou back) pour rester centré
    const firstChild = topbar.firstElementChild;
    if (firstChild && firstChild.nextSibling) {
      topbar.insertBefore(picker, firstChild.nextSibling);
    } else {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Restaurer le thème couleur sauvegardé
  injectColorThemePicker();
  const savedColorTheme = localStorage.getItem('deentag_color_theme') || 'gold';
