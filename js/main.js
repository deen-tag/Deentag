...
function setAudioAutoPlay(val) {
  ...
  // Mettre à jour tous les boutons audio-mode visibles
...
function toggleAccordion(id) {
  ...
  if (!isOpen) {
    ...
    setTimeout(() => {
      const topBar = document.querySelector('.top-bar, .inv-topbar');
      const topBarH = topBar ? topBar.offsetHeight : 56;
      const rect = clicked.getBoundingClientRect();
      const scrollTarget = window.scrollY + rect.top - topBarH - 10;
      window.scrollTo({ top: scrollTarget, behavior: 'smooth' });
    }, 460);
    // Lancer l'audio automatiquement une fois l'animation terminée (si mode auto)
    const body = clicked.querySelector('.accordion-body');
    if (body) {
      const onTransitionEnd = (e) => {
        if (e.propertyName !== 'max-height') return;
        body.removeEventListener('transitionend', onTransitionEnd);
        if (!getAudioAutoPlay()) return;
        const audioMap = getAudioMap();
        const inner = clicked.querySelector('.accordion-inner');
        const hasSunnah = inner && !!inner.querySelector('.sunnah-item');
        if (!hasSunnah && audioMap[id]) {
          const audioFile = audioMap[id].ar;
          const btn = document.getElementById('tts-ar-' + id);
          if (btn && audioFile) playAudio(id, 'ar', btn, audioFile);
        }
      };
...
function playAudio(accId, type, btn, audioFile) {
  ...
  if (type === 'tr') {
    ...
    utt.lang = targetLang;
    utt.rate = 0.88;
    // Chercher une voix masculine pour la langue cible
    const allVoices = window.speechSynthesis.getVoices();
    const maleKeywords = ['male','man','homme','hombre','david','jorge','carlos','thomas','nicolas','albert','pierre','google uk english male','microsoft david','microsoft pablo','microsoft claude'];
    const langVoices = allVoices.filter(v => v.lang.startsWith(targetLang.split('-')[0]));
    const maleVoice = langVoices.find(v => maleKeywords.some(k => v.name.toLowerCase().includes(k)))
                   || langVoices.find((v, i) => !['female','femme','mujer','woman'].some(k => v.name.toLowerCase().includes(k)) && i % 2 === 0)
                   || langVoices[0];
...
// ===== SETTINGS PANEL — centré sous la toolbar =====
function toggleAccSettings(accId) {
  ...
  closeAllSettings();
  if (!isOpen) {
    // Positionner juste sous la toolbar
    const toolbar = document.querySelector('.inv-topbar');
    const topbarH = toolbar ? toolbar.offsetHeight : 56;
    panel.style.top = (topbarH + 8) + 'px';

    // Appliquer la couleur kids si mode actif
    if (document.body.classList.contains('kids')) {
      const acc = document.getElementById(accId);
      if (acc) {
        const kidsColor = getComputedStyle(acc).getPropertyValue('--kids-color').trim();
        if (kidsColor) {
          panel.style.setProperty('--kids-panel-color', kidsColor);
        }
      }
    } else {
...
function injectAccordionControls() {
  ...
  document.querySelectorAll('.accordion').forEach(acc => {
    ...
    // === BARRE RETOUR + RÉGLAGES + PARTAGE ===
    const actions = document.createElement('div');
    actions.className = 'acc-actions';
    const backHref = document.querySelector('.back-btn') ? document.querySelector('.back-btn').getAttribute('href') : 'categorie.html';
    ...
    // Ajouter l'option auto/manuel audio
    const isAuto = getAudioAutoPlay();
    const audioModeLabel = i18n[currentLang]['audio-mode'] || 'Lecture audio';
    const audioAutoLabel = i18n[currentLang]['audio-auto'] || 'Auto';
    const audioManualLabel = i18n[currentLang]['audio-manual'] || 'Manuel';
    pillsHTML += '<div class="size-block" style="margin-top:10px;padding-top:10px;border-top:1px solid rgba(201,168,76,0.2)">' +
      '<span class="size-block-label">' + audioModeLabel + '</span>' +
      '<div class="size-pills">' +
        '<button class="size-pill audio-mode-pill' + (isAuto ? ' active' : '') + '" data-audio-val="auto" onclick="event.stopPropagation();setAudioAutoPlay(true)">' + audioAutoLabel + '</button>' +
        '<button class="size-pill audio-mode-pill' + (!isAuto ? ' active' : '') + '" data-audio-val="manual" onclick="event.stopPropagation();setAudioAutoPlay(false)">' + audioManualLabel + '</button>' +
      '</div>' +
    '</div>';
  ...
  // ===== BANDE STICKY TITRE + ✕ en mode direct =====
  const params = new URLSearchParams(window.location.search);
  const openAcc = params.get('open');
  if (openAcc) {
    document.querySelectorAll('.direct-acc-header').forEach(el => el.remove());
    document.querySelectorAll('.direct-acc-wrapper').forEach(el => {
      const parent = el.parentNode;
      while (el.firstChild) parent.insertBefore(el.firstChild, el);
      parent.removeChild(el);
    });
    const acc = document.getElementById(openAcc);
    if (acc) {
      const titleEl = acc.querySelector('.accordion-title');
      const stickyHeader = document.createElement('div');
      stickyHeader.className = 'direct-acc-header';
      stickyHeader.innerHTML =
        '<div class="direct-acc-title">' + (titleEl ? titleEl.innerHTML : '') + '</div>' +
        '<button class="direct-close-btn" onclick="closeToHome()">&#10005;</button>';

      // Wrapper commun titre + contenu
      const wrapper = document.createElement('div');
      wrapper.className = 'direct-acc-wrapper';
      acc.parentNode.insertBefore(wrapper, acc);
      wrapper.appendChild(stickyHeader);
      wrapper.appendChild(acc);

      // Autoplay audio en mode direct (pas d'animation donc pas de transitionend)
      if (getAudioAutoPlay()) {
        setTimeout(() => {
          const audioMap = getAudioMap();
          const inner = acc.querySelector('.accordion-inner');
          const hasSunnah = inner && !!inner.querySelector('.sunnah-item');
          if (!hasSunnah && audioMap[openAcc]) {
            const audioFile = audioMap[openAcc].ar;
            const btn = document.getElementById('tts-ar-' + openAcc);
            if (btn && audioFile) playAudio(openAcc, 'ar', btn, audioFile);
          }
        }, 600);
      }
    }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Sticky header scroll detection (mode direct NFC)
  const stickyHeader = document.querySelector('.direct-acc-header');
  if (stickyHeader) {
    const topbar = document.querySelector('.inv-topbar');
    const topbarH = topbar ? topbar.offsetHeight : 56;
    const sentinel = document.createElement('div');
    sentinel.style.cssText = 'position:absolute;top:0;height:1px;width:1px;pointer-events:none;';
    stickyHeader.parentNode.insertBefore(sentinel, stickyHeader);
    const observer = new IntersectionObserver(
      ([entry]) => stickyHeader.classList.toggle('is-sticky', !entry.isIntersecting),
  ...
  // Auto-open accordion if navigating from index flip card
  const targetAcc = sessionStorage.getItem('deentag_open_acc');
  if (targetAcc) {
    sessionStorage.removeItem('deentag_open_acc');
    const el = document.getElementById(targetAcc);
    if (el) {
      // Small delay to let page render/animate in
      setTimeout(() => { toggleAccordion(targetAcc); }, 350);
    }
  }

  // Restaurer l'ordre sauvegardé si on est sur l'index
  restoreSavedOrder();
});

// ===== RÉORGANISATION STYLE iOS =====
...
function toggleReorder() {
  ...
  grid.classList.toggle('edit-mode', isReorderMode);
  btn.classList.toggle('editing', isReorderMode);

  const svgEl = btn.querySelector('.reorder-circle svg');
  if (isReorderMode) {
    // Garder les 6 points, le CSS s'occupe de l'illumination
  } else {
    svgEl.innerHTML = `
  ...
  const cards = grid.querySelectorAll('.icon-card:not(.coming-soon)');
  cards.forEach(card => {
    if (isReorderMode) {
      initDragCard(card);
    } else {
...
function getCards() {
  return [...document.querySelectorAll('#icons-grid .icon-card:not(.coming-soon)')];
}

function moveCard(draggedCard, targetCard) {
  const grid = document.getElementById('icons-grid');
  ...
  // Insérer la carte glissée à la position cible (les autres se poussent)
  if (fromIdx < toIdx) {
...
function initDragCard(card) {
  ...
  card.ondragover = (e) => {
    e.preventDefault();
    if (dragSrc && card !== dragSrc && !card.classList.contains('coming-soon')) {
      moveCard(dragSrc, card);
    }
  };
  card.ondragend = () => {
  ...
  card.ontouchstart = (e) => {
    ...
    touchDragEl = card;
    lastTargetIndex = getCards().indexOf(card);
    ...
    touchClone.style.cssText = `
      ...
      transition:transform 0.15s; border-radius:16px;
    `;
  ...
  card.ontouchmove = (e) => {
    ...
    const target = el ? el.closest('#icons-grid .icon-card') : null;

    if (target && target !== touchDragEl && !target.classList.contains('coming-soon')) {
      const newIdx = getCards().indexOf(target);
      if (newIdx !== lastTargetIndex) {
        moveCard(touchDragEl, target);
...
function saveOrder() {
  const grid = document.getElementById('icons-grid');
  if (!grid) return;
  const order = [...grid.querySelectorAll('.icon-card')].map(c => c.getAttribute('data-id'));
  localStorage.setItem('deentag_icon_order', JSON.stringify(order));
}

function restoreSavedOrder() {
  const grid = document.getElementById('icons-grid');
  if (!grid) return;
  const saved = localStorage.getItem('deentag_icon_order');
  if (!saved) return;
  try {
...
// Mettre à jour le bouton cercle thème
function updateThemeCircleBtn() {
  const btn = document.getElementById('theme-circle-btn');
  if (!btn) return;
