...
function toggleSettings(accId) {
  ...
  if (!isOpen) {
    if (btn) {
      ...
      const panelW = 280;
      const panelH = 220; // hauteur estimée du panel
      ...
      // Position verticale : toujours au-dessus du bouton
      const topAboveBtn = rect.top - panelH - 6;

      panel.style.left = left + 'px';
      panel.style.bottom = '';
      panel.style.top = topAboveBtn + 'px';
      ...
      // Si le panel dépasse en haut de l'écran, scroll la page vers le bas
      if (topAboveBtn < 8) {
        const scrollNeeded = 8 - topAboveBtn;
...
function injectAccordionControls() {
  ...
  document.querySelectorAll('.accordion').forEach(acc => {
    ...
    function buildPillsHTML(types) {
      let html = '<div class="settings-panel-title" data-i18n-panel="taille">TAILLE DU TEXTE</div>';
