...
function toggleAccSettings(accId) {
  ...
  if (!isOpen) {
    ...
    // Positionner à gauche du bouton ⚙️
    const triggerBtn = document.querySelector('#' + accId + ' .dua-action-icon[title="Réglages"]');
    if (triggerBtn) {
...
function injectAccordionControls() {
  ...
  document.querySelectorAll('.accordion').forEach(acc => {
    ...
    if (hasAudio) {
      const svgAR = '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/></svg>';
      bar.innerHTML =
        '<div class="audio-solo-bar">' +
          '<button class="audio-solo-btn" id="tts-ar-' + accId + '" onclick="playAudio(\'' + accId + '\', \'ar\', this, \'' + audioFile + '\')">' +
            svgAR +
          '</button>' +
        '</div>';
    }
