...
function speakElement(el, langCode, rawOverride, onEnd) {
  ...
  // boundary se déclenche à chaque mot — on cherche le span correspondant
  let wordIndex = 0;
  utt.onboundary = (e) => {
    if (e.name !== 'word') return;
    // Calculer quel mot on est en comptant les espaces avant charIndex
    const before = rawText.substring(0, e.charIndex);
    const wi = before.split(/\s+/).filter(w => w.length > 0).length;
    spans.forEach(s => s.classList.remove('active-word'));
    if (spans[wi]) spans[wi].classList.add('active-word');
    wordIndex = wi;
  };
...
function injectAccordionControls() {
  ...
  document.querySelectorAll('.accordion').forEach(acc => {
    ...
    if (!hasSunnah) {
      ...
      slidersHTML =
        '<div class="slider-row">' +
          '<span class="slider-label" data-size-type="fr">Taille</span>' +
          '<input type="range" class="size-slider" data-size-type="fr" min="' + SIZE_MIN.fr + '" max="' + SIZE_MAX.fr + '" value="' + sizes.fr + '" oninput="saveAndApplySize(\'fr\', this.value)">' +
          '<span class="size-val" data-size-type="fr">' + sizes.fr + '</span>' +
        '</div>';
    }
