...
function renderJuzCards() {
  ...
  JUZ_DATA.forEach(juz => {
    ...
    card.innerHTML = `
      ${svgBg}
      <div class="juz-number-ar">${juz.ar}</div>
      <div class="juz-divider"></div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (search) {
    search.addEventListener('input', () => {
      ...
      if (juz) renderSurahList(juz.sourahs, search.value);
    });
  }
});
...
function openSurah(surahId) {
  ...
  setTimeout(() => {
    ...
    const lang = getLang();
    document.getElementById('surahBackBtn').textContent = '← ' + (BACK_WORD[lang] || 'Retour');
...
async function loadSurah(surahId) {
  ...
  try {
    ...
    renderSurahContent(surah, data, lang);
...
function renderSurahContent(surah, data, lang) {
  const scroll = document.getElementById('surahScroll');
  if (!scroll) return;
    ...
    html += `
      <div class="verse-block" id="verse-${vNum}">
        <div class="verse-number">
          ...
          <button class="verse-copy-btn" onclick="copyVerse(${vNum})" title="Copier">
...
function playVerseAudio(verseNum) {
  ...
  const url = `https://cdn.islamic.network/quran/audio/128/ar.alafasy/${(currentSurahId - 1) * 1000 + verseNum}.mp3`;
  // URL format corrigé
...
function openSettings() {
  ...
  if (!isOpen) {
    ...
    settingsTimer = setTimeout(() => panel.classList.remove('open'), 5000);
  }
}
