...
function openSurah(surahId, startVerse) {
  ...
  // Déterminer quelle vue est actuellement visible
  const activeView = (navView && navView.style.display !== 'none') ? navView : listView;
  ...
  setTimeout(() => {
    ...
    if (navView) { navView.style.display = 'none'; navView.style.opacity = ''; navView.style.transition = ''; }
...
function renderSurahContent(surah, data, lang, startVerse) {
  ...
  let html = `
    <div class="surah-view-header fade-in">
      ...
      <div class="surah-view-meta">${typeLabel} · ${surah.verses} ${VERSE_LABEL[lang] || 'versets'}</div>
    </div>
...
async function startKaraoke(fromVerse) {
  ...
  if (playBtn) { playBtn.classList.add('playing'); playBtn.innerHTML = SVG_PAUSE; }
...
const SVG_PAUSE = `<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>`;
...
function renderNavList(type, lang) {
  ...
  if (type === 'full') {
    ...
    grid.className = 'nav-juz-grid';
    JUZ_DATA.forEach(juz => {
      const card = document.createElement('div');
      card.className = 'juz-card';
      card.onclick = () => {
        closeNavAndOpenJuz(juz.juz);
      };
      const svgBg = `<svg class="juz-ornament-bg" viewBox="0 0 100 100" fill="none"><circle cx="50" cy="50" r="45" stroke="#C9A84C" stroke-width="0.8"/><circle cx="50" cy="50" r="35" stroke="#C9A84C" stroke-width="0.5"/><line x1="5" y1="50" x2="95" y2="50" stroke="#C9A84C" stroke-width="0.5"/><line x1="50" y1="5" x2="50" y2="95" stroke="#C9A84C" stroke-width="0.5"/><circle cx="50" cy="50" r="8" stroke="#C9A84C" stroke-width="0.8"/></svg>`;
      card.innerHTML = `${svgBg}<div class="juz-watermark">${juz.ar}</div><div class="juz-label">${JUZ_WORD[lang] || 'Juz'} ${juz.juz}</div><div class="juz-range">${juz.range}</div>`;
      grid.appendChild(card);
    });
