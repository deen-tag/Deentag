...
function openJuzSheet(juzNum) {
  ...
  sheet.classList.add('sheet--partial');
...
function closeSheet(fromSwipe) {
  ...
  if (fromSwipe) {
    ...
    setTimeout(() => {
      sheet.classList.remove('open', 'sheet--partial', 'sheet--full');
...
function openSurah(surahId, startVerse) {
  ...
  setTimeout(() => {
    ...
    sheet.classList.remove('sheet--partial');
...
function backToList() {
  ...
  // Détermine la vue de retour : navView si on venait d'une nav, listView sinon
  const goToNav = currentNavType && currentNavType !== 'juz';
  ...
  setTimeout(() => {
    ...
    // Préparer la vue de retour
    if (goToNav) {
      navView.style.display  = 'flex';
      navView.style.opacity  = '0';
      listView.style.display = 'none';
      // Repopuler la navView pour éviter la vue vide
      renderNavList(currentNavType, lang);
    } else {
    ...
    requestAnimationFrame(() => {
      ...
      const targetView = goToNav ? navView : listView;
      targetView.style.transition = 'opacity 0.2s ease';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  sheet.addEventListener('touchmove', e => {
    ...
    const scrollEl = currentSurahId
      ? document.getElementById('surahScroll')
      : (document.getElementById('navList') || document.getElementById('surahList'));
    if (scrollEl && scrollEl.scrollTop > 5) { startY = e.touches[0].clientY; swipeDY = 0; return; }
    ...
    if (currentSurahId) {
      ...
      const activeView = (nv && nv.style.display !== 'none') ? nv : lv;
      if (activeView) activeView.style.opacity = String(1 - progress);
    }
