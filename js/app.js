...
const LANGS = ['fr', 'en', 'es']; // ← ex: ajouter 'de', 'it', 'nl', 'tr'
...
function buildPillsHTML(types) {
  ...
  const isDyslexic = (localStorage.getItem('deentag_font') || 'default') === 'dyslexic';
...
function applyFont(font) {
  document.body.classList.toggle('font-dyslexic', font === 'dyslexic');
  localStorage.setItem('deentag_font', font);
...
function toggleDyslexicFont() {
  const current = localStorage.getItem('deentag_font') || 'default';
...
function showWelcomeIfNeeded() {
  ...
  bubble.innerHTML =
    ...
    '<span class="welcome-text">' +
      LANGS.map(l => '<span class="lang-block" data-lang-block="' + l + '">' + ({
...
function renderSheetItems(cat, filter) {
  ...
  if (filtered.length === 0) {
    list.innerHTML = '<div class="bs-no-result">' +
      LANGS.map(l => '<span class="lang-block" data-lang-block="' + l + '">' + ({ fr:'Aucun résultat', en:'No results', es:'Sin resultados' }[l]||'') + '</span>').join('') +
      '</div>';
...
function renderDua(cat, accId) {
  ...
  if (isSunnah) {
    ...
    acc.innerHTML =
      '<div class="accordion-header" style="cursor:default"><div class="accordion-title">' +
        LANGS.map(l => '<span class="lang-block" data-lang-block="' + l + '">' + (dua.titre[l] || '') + '</span>').join('') +
      '</div></div>' +
      '<div class="accordion-body" style="max-height:none;overflow:visible"><div class="accordion-inner">' +
        LANGS.map(l => '<div class="lang-block" data-lang-block="' + l + '">' + (sunHtml[l] || '') + '</div>').join('') +
  ...
  closing.innerHTML = LANGS.map(l => '<span class="lang-block" data-lang-block="' + l + '">' + (cd[l] || '') + '</span>').join('');
...
function openDua(cat, accId) {
  ...
  setTimeout(() => {
    ...
    if (backLabel) backLabel.textContent = DUAS[cat].meta.titre[lang] || DUAS[cat].meta.titre[LANGS[0]];
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const savedFont = localStorage.getItem('deentag_font') || 'default';
  applyFont(savedFont);
