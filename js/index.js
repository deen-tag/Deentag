...
function getSheetItems(cat) {
  if (!window.DUAS || !DUAS[cat]) {
    console.warn('DUAS non chargé ou catégorie introuvable:', cat, window.DUAS);
    return [];
  }
  const page = DUAS[cat];
...
function openSheet(cat) {
  ...
  // S'assurer que DUAS est chargé
  if (!window.DUAS || !DUAS[cat]) {
    console.error('DUAS non disponible pour:', cat);
    return;
  }

  // Afficher vue liste d'abord
  ...
  document.getElementById('bsTitle').textContent = meta.titre[lang] || meta.titre.fr;
