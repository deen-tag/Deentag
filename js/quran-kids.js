  ...
  KIDS_CARD_IDS.forEach((cardId, i) => {
    ...
    if (cardId === 'all') {
      ...
      card.innerHTML = `
        ...
        <div class="kids-card-ar" style="font-size:22px;">📚</div>
...
function kidsOpenSurah(surahId) {
  ...
  if (headerEl) {
    ...
    headerEl.querySelector('.surah-header-emoji').textContent = KIDS_EMOJIS[KIDS_CARD_IDS.indexOf(surahId) % KIDS_EMOJIS.length];
...
function kidsShowVerse(verseNum) {
  ...
  // Basmala uniquement sur verset 1 (sauf Al-Fatiha qui l'inclut, et At-Tawba)
  const showBasmala = (verseNum === 1 && surah.id !== 9 && surah.id !== 1);
  const basmalaHtml = showBasmala
    ? `<div class="kids-basmala">بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ</div>` : '';
    ...
    <div class="kids-dua-card verse-card-anim" style="border-color:${color}44">
      ...
      <div class="kids-dua-body">
        ...
        <div class="kids-audio-bar">
          <button class="kids-audio-btn" id="verseAudioBtn" style="color:${color}" onclick="kidsToggleVerseAudio()">
            <span>🔊</span>
          </button>
        </div>
      </div>
...
function kidsGoPrev() {
  if (kidsCurrentVerse > 1) kidsShowVerse(kidsCurrentVerse - 1);
}

function kidsGoNext() {
  if (!kidsVerseData) return;
  if (kidsCurrentVerse < kidsVerseData.total) kidsShowVerse(kidsCurrentVerse + 1);
}
...
function kidsStopAudio() {
  ...
  const btn = document.getElementById('verseAudioBtn');
  if (btn) { btn.classList.remove('playing'); btn.querySelector('span').textContent = '🔊'; }
}
...
function kidsRenderProgress(surah) {
  ...
  if (surah.verses <= 20) {
    ...
    counter.className = 'kids-verse-counter';
    counter.id = 'verseCounter';
    counter.innerHTML = `<span id="counterCurrent">1</span> / ${surah.verses}`;
...
function kidsUpdateProgress(verseNum) {
  ...
  if (surah.verses <= 20) {
    // Allumer les étoiles jusqu'au verset actuel
    for (let v = 1; v <= surah.verses; v++) {
      const star = document.getElementById('star-v-' + v);
      if (star) star.classList.toggle('earned', v <= verseNum);
    }
  } else {
