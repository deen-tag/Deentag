...
const COLORS1 = {
  ...
  nacre:'#D8D0C0', aurore:'#D8A898', peche:'#D89878', ivoire:'#D8C8A8', 'or-blanc':'#C8B898'
};
...
const COLORS2 = {
  brutalist:'#1A1A1A', 'neon-mosque':'#00CC66', verre:'#2A8060', manuscript:'#7A5030',
  aurora:'#28B890', 'mono-green':'#008844', marble:'#2A6A50', terminal:'#00CC44',
  ...
  cinema:'#50A870', 'sand-dune':'#888020', 'deep-sea':'#00A878', spring:'#38A840',
  'charcoal-rose':'#3A8858', solar:'#287858', pewter:'#4A7860', abyssal:'#00D090',
  'clay-mint':'#2A9870', celestial:'#28A888'
};
...
function applyTheme1(name) {
  ...
  const active = document.querySelector(`.ct-dot[data-ct="${name}"]`);
...
function applyTheme2(name) {
  ...
  document.querySelectorAll('.ct-dot').forEach(d => d.classList.remove('active'));
  const active = document.querySelector(`.ct-dot[data-ct2="${name}"]`);
  if (active) active.classList.add('active');
}

function makeDot(num, label, color, onClick) {
  ...
  // Numéro affiché dans le dot
  const lbl = document.createElement('span');
  lbl.textContent = num;
  lbl.style.cssText = `
    position:absolute; inset:0;
    display:flex; align-items:center; justify-content:center;
    font-size:8px; font-weight:700; font-family:sans-serif;
...
function injectPicker() {
  ...
  wrap.style.cssText = `
    width:100%; background:var(--bg,#0F0D08);
  ...
  // Série 1
  THEMES1.forEach((t, i) => {
    const dot = makeDot(i + 1, t, COLORS1[t], () => applyTheme1(t));
    dot.dataset.ct = t;
  ...
  sep.style.cssText = 'width:2px;height:20px;background:rgba(128,128,128,0.35);flex-shrink:0;border-radius:2px;margin:0 4px';
  picker.appendChild(sep);

  // Série 2
  THEMES2.forEach((t, i) => {
    const dot = makeDot(i + 31, t, COLORS2[t], () => applyTheme2(t));
    dot.dataset.ct2 = t;
    picker.appendChild(dot);
  });
...
style.textContent = `
  .ct-dot.active {
    ...
    box-shadow: 0 0 0 2px var(--bg,#0F0D08), 0 0 0 4px rgba(255,255,255,0.6) !important;
  }
  .ct-dot:active { transform: scale(0.85) !important; }
  .ct-wrap::-webkit-scrollbar { display:none; }
`;
...
window.addEventListener('DOMContentLoaded', () => {
  injectPicker();
  const saved2 = localStorage.getItem('deentag_color_theme2');
  const saved1 = localStorage.getItem('deentag_color_theme');
  if (saved2) applyTheme2(saved2);
  else applyTheme1(saved1 || 'islam');
});
