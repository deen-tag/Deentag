...
(function () {
  ...
  function openKidsProfileModal() {
    ...
    profiles.forEach(function(p) {
      ...
      div.innerHTML =
        '<div class="kids-profile-dome" style="cursor:pointer;">' + kidsDome(p, 52) + '</div>' +
        '<div class="kids-profile-name" style="cursor:pointer;">' + p.name + '</div>' +
        (isActive ? '<div class="kids-profile-check">✓</div>' : '') +
        '<div style="display:flex;gap:4px;margin-top:2px;">' +
          '<button onclick="event.stopPropagation();kidsEditProfile('' + p.id + '')" style="background:#BAE1FF;border:none;border-radius:20px;padding:3px 8px;font-size:11px;font-family:Baloo 2,cursive;font-weight:700;color:#1a4a6e;cursor:pointer;">✏️</button>' +
          '<button onclick="event.stopPropagation();kidsDeleteProfile('' + p.id + '')" style="background:#FFB3BA;border:none;border-radius:20px;padding:3px 8px;font-size:11px;font-family:Baloo 2,cursive;font-weight:700;color:#8B1a1a;cursor:pointer;">🗑️</button>' +
        '</div>';
      // Clic sur dôme/nom pour sélectionner
      div.querySelector('.kids-profile-dome').onclick = function() {
        setActiveId(p.id);
        updateKidsTabAvatar();
        closeKidsProfileModal();
        window.dispatchEvent(new CustomEvent('deentag:profileChanged', { detail:{ id:p.id } }));
      };
      div.querySelector('.kids-profile-name').onclick = function() {
        setActiveId(p.id);
        updateKidsTabAvatar();
        closeKidsProfileModal();
        window.dispatchEvent(new CustomEvent('deentag:profileChanged', { detail:{ id:p.id } }));
  ...
  function kidsEditProfile(id) {
    ...
    box.innerHTML =
      ...
      '<input id="kp-name" type="text" value="' + p.name + '" maxlength="12" ' +
        ...
        'box-sizing:border-box;margin-bottom:14px;outline:none;text-align:center;">' +
      '<div style="display:flex;gap:10px;">' +
        '<button onclick="var oldModal=document.getElementById('kidsProfileModal');if(oldModal)oldModal.remove();openKidsProfileModal();" style="flex:1;padding:13px;border-radius:50px;border:2px solid rgba(108,99,255,0.2);background:transparent;font-family:Baloo 2,cursive;font-weight:700;font-size:14px;color:#262B45;cursor:pointer;">Annuler</button>' +
