...
(function () {
  ...
  function updateKidsTabAvatar() {
    var icon = document.getElementById('kids-profile-tab-icon');
    var lbl  = document.getElementById('kids-profile-tab-label');
    if (!icon) return;
    var profiles = loadProfiles();
    var activeId = getActiveId();
  ...
  /* ── Mettre à jour l'avatar dans la tabbar kids (le bouton est créé par nav.js) ── */
  ...
  function init() {
    ...
    // Écouter l'événement émis par nav.js quand la tabbar est créée
    window.addEventListener('deentag:tabbarReady', function(e) {
      if (e.detail && e.detail.mode === 'kids') {
        injectKidsProfileTab();
      }
    });

    // Fallback : si la tabbar existe déjà au moment du chargement
