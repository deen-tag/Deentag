...
function applyLang(lang) {
  ...
  if (window.DT_refreshGreetingLang) window.DT_refreshGreetingLang();
}
...
function toggleTheme() {
  ...
  localStorage.setItem('deentag_theme_manual', '1');
}
...
function shareAccordion(accId) {
  ...
  text += '\n— Deentag\n🔗 https://deentag.vercel.app/' + (document.documentElement.getAttribute('lang')||'fr') + '/shop';
  if (navigator.share) {
...
window.DT_registerInit(() => {
  ...
  let savedTheme;
  if (localStorage.getItem('deentag_theme_manual')) {
    savedTheme = localStorage.getItem('deentag_theme') || 'day';
  } else {
    const hour = new Date().getHours();
    savedTheme = (hour >= 6 && hour < 20) ? 'day' : 'night';
    localStorage.setItem('deentag_theme', savedTheme);
  }
