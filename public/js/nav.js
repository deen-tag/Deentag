(function () {
  ...
  // Convertit les anciens noms de fichiers ('kids.html', 'index.html'...) en
  // routes Next.js préfixées par la langue courante ('/fr/kids', '/fr'...),
  // puisque chaque langue vit désormais sous sa propre URL.
  var ROUTE_MAP = {
    'index.html': '',
    'invocations.html': '/invocations',
    'quran.html': '/quran',
    'kids.html': '/kids',
    'quran-kids.html': '/quran-kids',
    'shop.html': '/shop'
  };
  function navRoute(href) {
    if (href === '#') return '#';
    var locale = document.documentElement.getAttribute('lang') || navGetLang();
    var route = ROUTE_MAP[href];
    if (route === undefined) return href;
    return '/' + locale + route;
  }
  window.navRoute = navRoute;
  ...
  function buildNav(cfg) {
    ...
    cfg.tabs.forEach(function (tab) {
      ...
      var routeHref = navRoute(tab.href);
      html +=
        '<a href="' + (isKidsExitTab ? '#' : routeHref) + '" class="tab' + (isActive ? ' active' : '') + '"' +
        (isActive ? ' aria-current="page"' : '') +
        (isKidsExitTab ? ' onclick="event.preventDefault();navConfirmExitKids(\'' + routeHref + '\')"' : '') +
